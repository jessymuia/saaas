...
//                            Mail::to($tenancyAgreement->tenant->email)
    ...
    public function createInvoiceAndTenancyBill($tenancyAgreement): void
    {
        ...
        $tenancyBill->amount = (Carbon::parse($tenancyAgreement->next_escalation_date)->diffInDays(now()->firstOfMonth()) / now()->daysInMonth) * $tenancyAgreement->amount;
        $tenancyBill->vat = $tenancyAgreement->amount * AppUtils::VAT_RATE;
        $tenancyBill->total_amount = $tenancyAgreement->amount + ($tenancyAgreement->amount * AppUtils::VAT_RATE);
