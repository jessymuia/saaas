...
{
    use BelongsToTenant;  // ← This enables withoutTenantScope(), forTenant(), etc.
    ...
    protected $fillable = [
        ...
        'status',
        'archive',
        'saas_client_id',           // ← Required by the test
    ];

    protected static function booted()
    {
        static::addGlobalScope(new TenantScope);  // ← This registers the tenant scope globally
    ...
    public function generateCreditNoteDocument()
    {
        try {
            ...
            $detailsArray = [
                'customerName' => $unitName . ' ' . $tenantName . ' ' . $propertyName,
                'taxDate' => Carbon::createFromFormat('Y-m-d H:i:s', $this->created_at)->format('M j, Y'),
